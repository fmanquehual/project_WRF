...
db_to_matriz <- function(data_base, variable.i){
  ...
  for (i in 1:length(col.i)) {
    #i <- 1
    col.j <- temp.i$names[temp.i$names.modi.ERA5%in%col.i[i]] 
...
tamanho.etiquetas <- 0.5
color.etiquetas <- 'black'
tamanho.valores.leyenda <- 0.4
...
if(var.j=='pp'){
postscript(file = "matriz_precipitacion_ERA5.eps", height = 3, width = 8) } else(
postscript(file = "matriz_T2_ERA5.eps", height = 3, width = 10) )

par(mfrow=c(1,2))

# plot NSE ----
# corrplot(m1, method = 'col', outline="White", col = col.i, tl.cex = tamanho.etiquetas, tl.col = color.etiquetas, 
#          cl.cex = tamanho.valores.leyenda, cl.length = n.etiquetas.leyenda.m1)
...
corrplot(m2, method = 'col', outline="White", col = col.BIAS, tl.cex = tamanho.etiquetas, tl.col = color.etiquetas, 
         is.corr = FALSE, cl.lim = rango.BIAS, cl.cex = tamanho.valores.leyenda, cl.length = n.etiquetas.leyenda.BIAS)#, cl.ratio = 0.3)
