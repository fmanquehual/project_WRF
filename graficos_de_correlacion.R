...
db_to_matriz <- function(data_base, variable.i){
  ...
  for (i in 1:length(col.i)) {
    #i <- 1  
    col.j <- temp.i$names[temp.i$names_modi%in%col.i[i]] 
...
letra <- c('(a)', '(b)', '(c)', '(d)', '(e)')
tipo.letra <- 2 # bold
posicion.letra.eje.y <- 0
tamanho.etiquetas <- 0.7
color.etiquetas <- 'black'
tamanho.valores.leyenda <- 0.7
...
if(var.plot == 'pp'){
postscript(file = "matriz_precipitacion.eps", height = 4.5, width = 13) }
  if(var.plot == 'rh'){
    postscript(file = "matriz_RH.eps", height = 4, width = 12) }
    if(var.plot == 't2'){
      postscript(file = "matriz_T2.eps", height = 6, width = 13) }
      if(var.plot == 'tmax'){
        postscript(file = "matriz_tmax.eps", height = 6, width = 13) }
        if(var.plot == 'tmin'){
          postscript(file = "matriz_tmin.eps", height = 6, width = 13) }
          if(var.plot == 'sp'){
            postscript(file = "matriz_SP.eps", height = 4, width = 12) }
par(mfrow=c(2,2))

# correlacion ----
corrplot(m1, method = 'col', outline="White", col = col.i, tl.cex = tamanho.etiquetas, tl.col = color.etiquetas, 
...
# KGE ----
corrplot(m2, method = 'col', outline="White", col = col.i, tl.cex = tamanho.etiquetas, tl.col = color.etiquetas, 
         cl.cex = tamanho.valores.leyenda, cl.length = n.etiquetas.leyenda.m1.m2.m3)
legend(pos.letra, legend = letra[2], bty="n", text.font=tipo.letra, y.intersp = posicion.letra.eje.y)

# NSE ----
# corrplot(m3, method = 'col', outline="White", col = col.i, tl.cex = tamanho.etiquetas, tl.col = color.etiquetas, 
#          cl.cex = tamanho.valores.leyenda, cl.length = n.etiquetas.leyenda.m1.m2.m3)

corrplot_modificado(m3, method = 'col', outline="White", col = col.i, tl.cex = tamanho.etiquetas, tl.col = color.etiquetas, 
         cl.cex = tamanho.valores.leyenda, cl.length = n.etiquetas.leyenda.m1.m2.m3, activar.etiquetas.caracteres=TRUE,
         nuevas.etiquetas.de.leyenda = etiquetas.m3)
legend(pos.letra, legend = letra[3], bty="n", text.font=tipo.letra, y.intersp = posicion.letra.eje.y)

# RSR ----
# corrplot(m4, method = 'col', outline="White", col = col.m4, tl.cex = tamanho.etiquetas, tl.col = color.etiquetas, 
#          is.corr = FALSE, cl.lim = c(0, 2), cl.cex = tamanho.valores.leyenda, cl.length = n.etiquetas.leyenda.m4)

corrplot_modificado(m4, method = 'col', outline="White", col = col.m4, tl.cex = tamanho.etiquetas, tl.col = color.etiquetas, 
         is.corr = FALSE, cl.lim = c(0, 2), cl.cex = tamanho.valores.leyenda, cl.length = n.etiquetas.leyenda.m4,
         activar.etiquetas.caracteres=TRUE, nuevas.etiquetas.de.leyenda = etiquetas.m4)
